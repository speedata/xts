...
aliases:
  - /manual/data-processing/data-files/
---
...
The data file can be structured however you like -- there's no required schema. The only rule is that it must be well-formed XML. Other formats (CSV, Excel) can be converted to XML using the [Lua filter](/manual/advanced/lua-filter).
...
See the [Execution model](/programming/execution-model) for the full processing model, [Records and dispatch](/programming/records-and-dispatch) for matching data to records, and [XPath in XTS](/programming/xpath) for the expression language.
