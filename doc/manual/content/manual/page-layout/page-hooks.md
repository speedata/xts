...
For static headers and footers there is a third option that needs no hook at all: the page margin boxes of a CSS `@page` rule, described in [Page setup from CSS](../master-pages#page-setup-from-css). They live in the page margin and never occupy grid cells, whereas a header placed in `<AtPageCreation>` at row 1 takes that row away from the content. Use a hook when the header needs a table, several paragraphs or values from the data.
