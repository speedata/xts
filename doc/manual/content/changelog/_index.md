...
- **New CSS property -bag-tab-stops sets tab stops in HTML text.**<br>
  A comma separated list of stops, each a length or a percentage of the line width, optionally followed by `start`, `end`, `center`, `decimal` or `decimal(",")` and a `leader()`, for example `-bag-tab-stops: 12mm, 100% end leader(dotted)` for a contents line. A tab in the text advances to the next stop. With stops set a tab survives any `white-space` mode, whitespace with a newline in it stays source formatting. The `leader()` function in `content` now also accepts the keywords `dotted`, `solid` and `space`.
