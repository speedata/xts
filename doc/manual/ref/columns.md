...
[Column](../column), [CopyOf](../copyof), [ForAll](../forall), [Loop](../loop), [Switch](../switch), [Value](../value)
...
## Remarks
The `\*` widths in the command “Column” allow dynamic cell widths. For that the total width of the table must be set and the attribute (on [Table](../table)) `stretch` must be set to `max`.
