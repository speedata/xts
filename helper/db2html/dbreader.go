...
import (
	...
	"github.com/speedata/xts/helper/fileutils"
...
var (
	...
	assetsTrim = regexp.MustCompile(`^(\.\./)*manual/assets(.*)$`)
	...
	for {
		...
		case xml.StartElement:
			...
			case "callout":
				calloutcounter++
				omitP = true
				curOutput.WriteString(`<p>` + string(rune('①'+calloutcounter-1)) + ` `)
			case "calloutlist":
				calloutcounter = 0
			case "co":
				thisid := attr(elt, "id")
				...
				curOutput.WriteString(string(rune('①' + conum - 1)))
			case "emphasis":
				class := ""
...
func (d *DocBook) WriteHTMLFiles(basedir string) error {
	...
	templatesdir := filepath.Join(d.cfg.Basedir(), "doc", "manual", "templates", "*.html")
	...
	assetsdir := filepath.Join(d.cfg.Basedir(), "doc", "manual", "assets")
