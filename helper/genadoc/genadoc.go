...
func GenerateAdocFiles(cfg *config.Config, lang string, mode ...string) error {
	...
	destpath := filepath.Join(cfg.Builddir, "manual", lang)
	...
	for _, v := range c.Commands() {
		...
		builddoc(c, v, lang, fullpath)
	}
	// wg.Wait()
	fmt.Println("cp", srcpath, "adoc-", lang, "->", destpath)
	...
	for _, chap := range cl.Chapter {
		...
		for _, entry := range chap.Entries {
			...
			e = ghIssueRegexp.ReplaceAllString(e, `https://github.com/speedata/xts/issues/$1[$1]`)
